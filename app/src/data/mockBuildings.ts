...
// 실 서비스 전환 시 이 파일을 국토교통부 실거래가 API + 브이월드 3D 건물 API 응답으로 교체.
...
function buildTrend(rand: () => number, priceEok: number): number[] {
  ...
  for (let i = 0; i < months; i++) {
    ...
    series.push(Math.max(v, priceEok * 0.55));
  }
...
function generateForDistrict(seed: DistrictSeed, idOffset: number, rand: () => number): Building[] {
  ...
  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 4; col++) {
      ...
      const prefix = NAME_PREFIXES[id % NAME_PREFIXES.length];

      buildings.push({
        id,
        name: `${prefix} ${String.fromCharCode(65 + (id % 26))}동`,
        type,
        district: seed.district,
        lng: seed.centerLng + (col - 1.5) * CELL,
        lat: seed.centerLat + (row - 1) * CELL,
        floors,
        heightMeters: Math.round(floors * 3.1),
        builtYear,
        areaPyeong,
        priceEok,
        jeonseEok,
...
export const mockBuildings: Building[] = DISTRICTS.flatMap((d, i) =>
  generateForDistrict(d, i * 100, rand)
);
